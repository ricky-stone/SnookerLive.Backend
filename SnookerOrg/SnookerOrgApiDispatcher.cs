...
public sealed class SnookerOrgApiDispatcher : BackgroundService
{
    ...
    private static async Task<string> WriteAsync(Channel<ApiRequest> channel, ApiRequest request, CancellationToken ct)
    {
    ...
    private async Task<string> CallWithRetry(ApiRequest request, CancellationToken ct)
    {
        ...
        for(int attempt = 0; attempt <= maxRetries; attempt++)
        {
            ...
            if(response.StatusCode == System.Net.HttpStatusCode.Forbidden)
            {
                ...
                _logger.LogWarning("403 Received, Retry {attempt}/{max}. Backing off {delay}ms", attempt + 1, maxRetries, delay);
